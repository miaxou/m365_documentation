...
if($orgid){
  ...
  $SafeAttachmentObj = foreach ($policy in $SafeAttachment) {
    [PSCustomObject]@{
      ...
      WhenChanged                = $policy.WhenChanged
      WhenCreated                = $policy.WhenCreated
      SentTo                     = ($SafeAttachmentRule | Where-Object {$_.Name -eq $policy.Name} ).SentTo
      SentToMemberOf             = ($SafeAttachmentRule | Where-Object {$_.Name -eq $policy.Name} ).SentToMemberOf 
      RecipientDomainIs          = ($SafeAttachmentRule | Where-Object {$_.Name -eq $policy.Name} ).RecipientDomainIs -join(' ')
      ExceptIfSentTo             = ($SafeAttachmentRule | Where-Object {$_.Name -eq $policy.Name} ).ExceptIfSentTo -join(' ')
      ExceptIfSentToMemberOf     = ($SafeAttachmentRule | Where-Object {$_.Name -eq $policy.Name} ).ExceptIfSentToMemberOf
      ExceptIfRecipientDomainIs  = ($SafeAttachmentRule | Where-Object {$_.Name -eq $policy.Name} ).ExceptIfRecipientDomainIs
  ...
  $SafeLinkObj = foreach ($policy in $SafeLink) {
    [PSCustomObject]@{
      ...
      WhenChanged                   = $policy.WhenChanged
      WhenCreated                   = $policy.WhenCreated
      SentTo                        = ($SafeLinkRule | Where-Object {$_.Name -eq $policy.Name} ).SentTo
      SentToMemberOf                = ($SafeLinkRule | Where-Object {$_.Name -eq $policy.Name} ).SentToMemberOf 
      RecipientDomainIs             = ($SafeLinkRule | Where-Object {$_.Name -eq $policy.Name} ).RecipientDomainIs -join(' ')
      ExceptIfSentTo                = ($SafeLinkRule | Where-Object {$_.Name -eq $policy.Name} ).ExceptIfSentTo -join(' ')
      ExceptIfSentToMemberOf        = ($SafeLinkRule | Where-Object {$_.Name -eq $policy.Name} ).ExceptIfSentToMemberOf
      ExceptIfRecipientDomainIs     = ($SafeLinkRule | Where-Object {$_.Name -eq $policy.Name} ).ExceptIfRecipientDomainIs
