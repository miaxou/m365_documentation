...
$FlexAssetName = "Azure AD Settings"
$Description = "Documentation for all AAD related settings"
...
$AzureADSettings = Get-MsolCompanyInformation -TenantId $customer.TenantID | select-object @{ Name = 'CompanyName'; Expression = { $_.'DisplayName' } },
...
$CAPolicyObj = foreach ($cap in $ConditionalAccessPolicyOutput) {
   [PSCustomObject]@{
        ...
        includeUsers                                = ($cap.conditions.users.includeUsers | ForEach-Object { Get-UserNameFromId -Users $UserListOutput -id $_ }) -join(" ")
        excludeUsers                                = ($cap.conditions.users.excludeUsers | ForEach-Object { Get-UserNameFromId -Users $UserListOutput -id $_ }) -join(" ")
        includeGroups                               = ($cap.conditions.users.includeGroups | ForEach-Object { Get-GroupNameFromId -Groups $GroupListOutput -id $_ }) -join(" ")
        excludeGroups                               = ($cap.conditions.users.excludeGroups | ForEach-Object { Get-GroupNameFromId -Groups $GroupListOutput -id $_ }) -join(" ")
...
@{
    ...
    attributes = @{
        traits = @{
            ...
            "azure-ad-settings"             = ($AzureADSettings | convertto-html -Fragment  | out-string)
            "conditional-access-policies"   = ($CAPolicyObj | convertto-html -Fragment | out-string)
            "named-locations"               = ($NamedLocations | convertto-html -Fragment  | out-string)
            "legacy-auth-sign-in"           = ($LegacyAuthSignIn | convertto-html -Fragment  | out-string)
        }
    }
}
...
if (!$ExistingFlexAsset) {
    ...
    write-host "Creating AAD setings for $($customer.name) into IT-Glue" -ForegroundColor Green
    New-ITGlueFlexibleAssets -data $FlexAssetBody
}
else {
    write-host "Updating AAD Settings  for $($customer.name) into IT-Glue"  -ForegroundColor Yellow
